"""
BraTS2023_PED dataset loader for pediatric brain tumor MRI segmentation.
Handles NIfTI 3D volumes and extracts 2D slices for SAM3/MedSAM3 evaluation.

BraTS2023_PED Labels:
- NC (label 1): Non-Enhancing Core - 100% of cases
- ED (label 2): Edema - 57.6% of cases
- ET (label 3): Enhancing Tumor - 88.9% of cases
"""
...
BRATS_PED_ROOT = Path("/shared/BIOE486/SP25/dataset/BrainMVP/BrainMVP-ds/BraTS2023_PED/BraTS2023-PED-Training")
...
# Text prompts for pediatric brain tumor segmentation
BRATS_PED_PROMPTS = {
...
@dataclass
class BraTSPEDSample:
    """A single BraTS-PED dataset sample (2D slice)."""
    image: np.ndarray          # RGB image (H, W, 3)
    gt_mask: np.ndarray        # Binary mask (H, W)
    case_id: str               # e.g., "BraTS-PED-00002-000"
...
def get_case_ids(
    ...
    Returns:
        List of case IDs
    """
    all_cases = sorted([d.name for d in BRATS_PED_ROOT.iterdir() if d.is_dir()])
...
def load_brats_ped_sample(
    case_id: str,
    contrast: str = "t1c",
    tumor_label: Optional[str] = None,
    text_prompt: str = "Brain Tumor",
    slice_idx: Optional[int] = None
) -> Optional[BraTSPEDSample]:
    """
    Load a single sample from a BraTS-PED case.
    ...
    Returns:
        BraTSPEDSample or None if loading fails
    """
    case_dir = BRATS_PED_ROOT / case_id
...
def get_cases_with_all_labels(max_samples: int = 10) -> List[str]:
    """
    Find BraTS-PED cases where NC, ED, and ET are all present in the middle slice.

    Note: ED (Edema) is only present in ~57.6% of cases.
    ...
    for case_id in all_cases:
        ...
        seg_path = BRATS_PED_ROOT / case_id / f"{case_id}-seg.nii.gz"
        if not seg_path.exists():
            continue
...
def load_brats_ped_dataset(
    ...
    seed: int = 42
) -> Iterator[BraTSPEDSample]:
    """
    Generator that yields BraTS-PED samples for middle slices.
...
def load_brats_ped_by_contrast(
    contrast: str,
    max_samples: Optional[int] = None,
    sample_ratio: float = 0.2,
    seed: int = 42
) -> Iterator[BraTSPEDSample]:
    """Load BraTS-PED samples for a single contrast."""
    return load_brats_ped_dataset(
        contrasts=[contrast],
...
def load_brats_ped_by_label(
    tumor_label: str,
    max_samples: Optional[int] = None,
    sample_ratio: float = 0.2,
    seed: int = 42
) -> Iterator[BraTSPEDSample]:
    """Load BraTS-PED samples for a specific tumor label."""
    prompt = BRATS_PED_PROMPTS.get(tumor_label, "Brain Tumor")
    return load_brats_ped_dataset(
        max_samples=max_samples,
        sample_ratio=sample_ratio,
...
def get_dataset_info() -> dict:
    """Get information about the BraTS-PED dataset."""
    all_cases = [d.name for d in BRATS_PED_ROOT.iterdir() if d.is_dir()]
    ...
    return {
        ...
        "root_path": str(BRATS_PED_ROOT),
    }


if __name__ == "__main__":
    # Test loading
    print("Testing BraTS2023_PED dataset loader...")
    print("=" * 60)
    ...
    for case_id in case_ids:
        print(f"\n{case_id}:")
        for contrast in CONTRASTS:
            sample = load_brats_ped_sample(case_id, contrast)
            if sample:
    ...
    for label in ["combined", "NC", "ED", "ET"]:
        samples = list(load_brats_ped_by_label(label, max_samples=2))
    ...
    print("\n" + "=" * 60)
    print("BraTS-PED loader test complete!")
