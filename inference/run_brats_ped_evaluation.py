#!/usr/bin/env python3
"""
SAM3 and MedSAM3 Evaluation on BraTS2023_PED (Pediatric) Dataset.

Evaluates pediatric brain tumor segmentation using TEXT PROMPTS ONLY:
- 4 MRI contrasts (t1n, t1c, t2w, t2f)
- 3 tumor labels (NC, ED, ET) + combined
- Middle slices only
...
Usage:
    ...
    # SAM3 baseline
    python run_brats_ped_evaluation.py --model sam3 --max-samples 20

    # MedSAM3 fine-tuned
    python run_brats_ped_evaluation.py --model medsam3 --max-samples 20

    # Quick test (5 samples)
    python run_brats_ped_evaluation.py --model sam3 --max-samples 5 --contrasts t1c
"""
...
from brats_ped_loader import (
    load_brats_ped_sample, get_case_ids, load_brats_ped_all_labels,
...
# Output directory
OUTPUT_DIR = Path(__file__).parent / "results" / "brats2023_ped"


# Text prompts to evaluate for pediatric brain tumors
PED_TEXT_PROMPTS = [
    "Brain Tumor",
...
def evaluate_text_prompts(
    ...
    Args:
        ...
        samples: List of BraTSPEDSample objects
...
def generate_report(results: Dict, output_dir: Path):
    ...
    csv_path = output_dir / f"{model_name}_ped_text_prompt_summary.csv"
    ...
    labels_csv_path = output_dir / f"{model_name}_ped_text_prompt_by_label.csv"
    df_labels.to_csv(labels_csv_path, index=False)
    print(f"Saved by-label results to: {labels_csv_path}")

    # === Detailed JSON ===
    json_path = output_dir / f"{model_name}_ped_detailed.json"
    with open(json_path, 'w') as f:
        json.dump(results, f, indent=2, default=str)
    print(f"Saved detailed results to: {json_path}")

    # === Markdown Report ===
    report_path = output_dir / f"{model_name}_ped_report.md"
    with open(report_path, 'w') as f:
        f.write(f"# {model_name.upper()} Evaluation on BraTS2023_PED (Pediatric)\n\n")
        f.write(f"**Generated**: {results['timestamp']}\n\n")
        f.write(f"**Dataset**: BraTS2023_PED (Pediatric Brain Tumors)\n\n")
        ...
        # Label explanations
        f.write("\n## Label Definitions\n\n")
        f.write("- **NC**: Non-Enhancing Core (present in 100% of cases)\n")
        f.write("- **ED**: Edema (present in ~57.6% of cases)\n")
        f.write("- **ET**: Enhancing Tumor (present in ~88.9% of cases)\n")

        f.write("\n## Notes\n\n")
        f.write("- Evaluated on middle slices of sampled cases\n")
        f.write("- Text prompts evaluated zero-shot (no prompt tuning)\n")
        f.write("- BraTS2023_PED contains pediatric gliomas\n")
        f.write("- Different from BraTS2023_MET (adult metastases)\n")
...
def create_comparison_report(sam3_json: Path, medsam3_json: Path, output_dir: Path):
    ...
    for contrast in CONTRASTS:
        ...
        for prompt in PED_TEXT_PROMPTS:
    ...
    with open(report_path, 'w') as f:
        f.write("# BraTS2023_PED Text Prompt-Only Evaluation Report\n\n")
        ...
        f.write("- Text prompts evaluated zero-shot (no prompt tuning)\n")
        f.write("- BraTS2023_PED: Pediatric brain gliomas\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="SAM3/MedSAM3 Evaluation on BraTS2023_PED Dataset (Text Prompts Only)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Maximum number of cases to sample (default: 20, ~20%%)"
    )
    parser.add_argument(
    ...
    if args.create_comparison:
        sam3_json = OUTPUT_DIR / "sam3_ped_detailed.json"
        medsam3_json = OUTPUT_DIR / "medsam3_ped_detailed.json"
    ...
    print("=" * 60)
    print("BraTS2023_PED Evaluation (Pediatric Brain Tumors)")
