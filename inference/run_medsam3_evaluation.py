#!/usr/bin/env python3
"""
MedSAM3 Fine-tuned Model Evaluation on Medical Image Datasets.

Evaluates MedSAM3 with custom checkpoints using both box prompts and text prompts on:
...
def evaluate_dataset(
    ...
    """
    Evaluate SAM3/MedSAM3 on a single dataset.
...
def generate_report(all_results: List[dict], output_dir: Path, model_name: str):
    """Generate evaluation report for MedSAM3."""
    ...
    # Save CSV
    csv_path = output_dir / f"{model_name}_evaluation_summary.csv"
    df_summary.to_csv(csv_path, index=False)
    print(f"\nSaved summary to: {csv_path}")

    # Save detailed JSON
    json_path = output_dir / f"{model_name}_evaluation_detailed.json"
    ...
    report_path = output_dir / f"{model_name}_evaluation_report.md"
    with open(report_path, 'w') as f:
        f.write(f"# {model_name} Medical Image Segmentation Evaluation\n\n")
...
def main():
    parser = argparse.ArgumentParser(description="MedSAM3 Fine-tuned Model Evaluation")
    parser.add_argument("--checkpoint", type=str, required=True,
                        help="Path to MedSAM3 checkpoint file")
    parser.add_argument("--model-name", type=str, default="medsam3",
                        help="Model name for output files (default: medsam3)")
    ...
    print("=" * 60)
    print(f"MedSAM3 Fine-tuned Model Evaluation")
    print("=" * 60)
    ...
    # Initialize SAM3 with custom checkpoint
    sam3 = SAM3Model(
    ...
    df_summary = generate_report(all_results, OUTPUT_DIR, args.model_name)
