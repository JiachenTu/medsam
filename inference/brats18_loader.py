"""
BraTS2018 dataset loader for High-Grade Glioma (HGG) MRI segmentation.
Handles NIfTI 3D volumes and extracts 2D slices for SAM3/MedSAM3 evaluation.

KEY DIFFERENCES FROM BraTS2021:
- File extension: .nii (uncompressed, NOT .nii.gz)
- Directory structure: HGG subdirectory
- Case count: 190 (HGG only)
- Case naming: Brats18_... (e.g., Brats18_2013_10_1, Brats18_CBICA_AAB_1)

SAME AS BraTS2021:
- Contrast names: t1, t1ce, t2, flair
- Label scheme: 0, 1, 2, 4 (NOT 0, 1, 2, 3 - label 4 for ET!)
- Volume size: 240 x 240 x 155

Labels:
- 0: Background
- 1: NCR (Necrotic/Non-Enhancing Tumor Core) - 100% of cases
- 2: ED (Peritumoral Edema) - ~99.6% of cases
- 4: ET (Enhancing Tumor) - ~90.5% of cases (NOTE: label 4, not 3!)
"""
...
# Dataset configuration - HGG subdirectory!
BRATS18_ROOT = Path("/shared/BIOE486/SP25/dataset/Brats/brats2018/MICCAI_BraTS_2018_Data_Training/HGG")

# MRI contrasts available (same as BraTS2021)
CONTRASTS = ["t1", "t1ce", "t2", "flair"]

# Tumor labels in segmentation mask
# NOTE: Label 4 for ET, NOT label 3! (same as BraTS2021)
TUMOR_LABELS = {
    0: "Background",
...
@dataclass
class BraTS18Sample:
    """A single BraTS2018 dataset sample (2D slice)."""
    image: np.ndarray          # RGB image (H, W, 3)
    gt_mask: np.ndarray        # Binary mask (H, W)
    case_id: str               # e.g., "Brats18_2013_10_1"
...
def load_nifti_volume(nifti_path: Path) -> np.ndarray:
    ...
    Args:
        nifti_path: Path to .nii file (uncompressed)
...
def get_case_ids(
    ...
    Returns:
        List of case IDs
    """
    all_cases = sorted([d.name for d in BRATS18_ROOT.iterdir() if d.is_dir()])
...
def load_brats18_sample(
    case_id: str,
    contrast: str = "t1ce",
    tumor_label: Optional[str] = None,
    text_prompt: str = "Brain Tumor",
    slice_idx: Optional[int] = None
) -> Optional[BraTS18Sample]:
    """
    Load a single sample from a BraTS2018 case.

    Args:
        case_id: Case identifier (e.g., "Brats18_2013_10_1")
    ...
    Returns:
        BraTS18Sample or None if loading fails
    """
    case_dir = BRATS18_ROOT / case_id

    # BraTS2018 file naming: {case_id}_{contrast}.nii (NO .gz!)
    mri_path = case_dir / f"{case_id}_{contrast}.nii"
...
def load_brats18_all_labels(
    ...
    # Load MRI volume - .nii extension (not .nii.gz)
    mri_path = case_dir / f"{case_id}_{contrast}.nii"
    if not mri_path.exists():
        return None
    ...
    # Load segmentation - .nii extension (not .nii.gz)
    seg_path = case_dir / f"{case_id}_seg.nii"
    if not seg_path.exists():
        return None
...
def get_cases_with_all_labels(max_samples: int = 10) -> List[str]:
    """
    Find BraTS2018 cases where NCR, ED, and ET are all present in the middle slice.
    ...
    for case_id in all_cases:
        # Load segmentation and check middle slice - .nii extension!
        seg_path = BRATS18_ROOT / case_id / f"{case_id}_seg.nii"
        if not seg_path.exists():
            continue
...
def load_brats18_dataset(
    ...
    seed: int = 42
) -> Iterator[BraTS18Sample]:
    """
    Generator that yields BraTS2018 samples for middle slices.
...
def load_brats18_by_contrast(
    contrast: str,
    max_samples: Optional[int] = None,
    sample_ratio: float = 0.2,
    seed: int = 42
) -> Iterator[BraTS18Sample]:
    """Load BraTS2018 samples for a single contrast."""
    return load_brats18_dataset(
        contrasts=[contrast],
...
def load_brats18_by_label(
    tumor_label: str,
    max_samples: Optional[int] = None,
    sample_ratio: float = 0.2,
    seed: int = 42
) -> Iterator[BraTS18Sample]:
    """Load BraTS2018 samples for a specific tumor label."""
...
def get_dataset_info() -> dict:
    """Get information about the BraTS2018 dataset."""
    all_cases = [d.name for d in BRATS18_ROOT.iterdir() if d.is_dir()]
    ...
    return {
        ...
        "root_path": str(BRATS18_ROOT),
    }


if __name__ == "__main__":
    # Test loading
    print("Testing BraTS2018 dataset loader...")
    print("=" * 60)
    ...
    for case_id in case_ids:
        print(f"\n{case_id}:")
        for contrast in CONTRASTS:
            sample = load_brats18_sample(case_id, contrast)
            if sample:
    ...
    for label in ["combined", "NCR", "ED", "ET"]:
        samples = list(load_brats18_by_label(label, max_samples=2))
    ...
    case_id = case_ids[0]
    data = load_brats18_all_labels(case_id, "t1ce")
    if data:
    ...
    print("\n" + "=" * 60)
    print("BraTS2018 loader test complete!")
