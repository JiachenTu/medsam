#!/usr/bin/env python3
"""
SAM3 and MedSAM3 Evaluation on BraTS2018 (High-Grade Glioma) Dataset.
...
- Case count: 190 (HGG only vs 1,251 in BraTS2021)
- Case naming: Brats18_... (e.g., Brats18_2013_10_1)
...
Usage:
    ...
    # SAM3 baseline
    python run_brats18_evaluation.py --model sam3 --max-samples 20

    # MedSAM3 fine-tuned
    python run_brats18_evaluation.py --model medsam3 --max-samples 20

    # Quick test (5 samples)
    python run_brats18_evaluation.py --model sam3 --max-samples 5 --contrasts t1ce
"""
...
from brats18_loader import (
    load_brats18_sample, get_case_ids, load_brats18_all_labels,
...
# Output directory
OUTPUT_DIR = Path(__file__).parent / "results" / "brats2018"


# Text prompts to evaluate for glioma (same as BraTS2021)
GLIOMA_TEXT_PROMPTS = [
    "Brain Tumor",
    "Glioma",
    "Enhancing Tumor",
...
def evaluate_text_prompts(
    ...
    Args:
        ...
        samples: List of BraTS18Sample objects
...
def generate_report(results: Dict, output_dir: Path):
    ...
    csv_path = output_dir / f"{model_name}_brats18_text_prompt_summary.csv"
    ...
    labels_csv_path = output_dir / f"{model_name}_brats18_text_prompt_by_label.csv"
    df_labels.to_csv(labels_csv_path, index=False)
    print(f"Saved by-label results to: {labels_csv_path}")

    # === Detailed JSON ===
    json_path = output_dir / f"{model_name}_brats18_detailed.json"
    with open(json_path, 'w') as f:
        json.dump(results, f, indent=2, default=str)
    print(f"Saved detailed results to: {json_path}")

    # === Markdown Report ===
    report_path = output_dir / f"{model_name}_brats18_report.md"
    with open(report_path, 'w') as f:
        f.write(f"# {model_name.upper()} Evaluation on BraTS2018 (High-Grade Glioma)\n\n")
        f.write(f"**Generated**: {results['timestamp']}\n\n")
        f.write(f"**Dataset**: BraTS2018 (High-Grade Glioma Brain Tumors)\n\n")
        ...
        # Label explanations
        f.write("\n## Label Definitions\n\n")
        f.write("- **NCR**: Necrotic/Non-Enhancing Tumor Core (label 1, 100% of cases)\n")
        f.write("- **ED**: Peritumoral Edema (label 2, ~99.6% of cases)\n")
        f.write("- **ET**: Enhancing Tumor (label 4, ~90.5% of cases) - NOTE: label 4, not 3!\n")

        f.write("\n## Notes\n\n")
        f.write("- Evaluated on middle slices of sampled cases\n")
        f.write("- Text prompts evaluated zero-shot (no prompt tuning)\n")
        f.write("- BraTS2018: High-grade glioma brain tumors (HGG only)\n")
        f.write("- Contrasts: T1, T1CE, T2, FLAIR\n")
        f.write("- File format: .nii (uncompressed)\n")
        f.write("- 190 total cases in dataset\n")
...
def create_comparison_report(sam3_json: Path, medsam3_json: Path, output_dir: Path):
    ...
    with open(report_path, 'w') as f:
        f.write("# BraTS2018 Text Prompt-Only Evaluation Report\n\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="SAM3/MedSAM3 Evaluation on BraTS2018 Dataset (Text Prompts Only)"
    )
    parser.add_argument(
    ...
    if args.create_comparison:
        sam3_json = OUTPUT_DIR / "sam3_brats18_detailed.json"
        medsam3_json = OUTPUT_DIR / "medsam3_brats18_detailed.json"
    ...
    print("=" * 60)
    print("BraTS2018 Evaluation (High-Grade Glioma Brain Tumors)")
