#!/usr/bin/env python3
"""
SAM3 and MedSAM3 Evaluation on BraTS2023_SSA (Sub-Saharan Africa) Dataset.
...
Usage:
    ...
    # SAM3 baseline
    python run_brats_ssa_evaluation.py --model sam3 --max-samples 15

    # MedSAM3 fine-tuned
    python run_brats_ssa_evaluation.py --model medsam3 --max-samples 15

    # Quick test (5 samples)
    python run_brats_ssa_evaluation.py --model sam3 --max-samples 5 --contrasts t1c
"""
...
from brats_ssa_loader import (
    load_brats_ssa_sample, get_case_ids, load_brats_ssa_all_labels,
...
# Output directory
OUTPUT_DIR = Path(__file__).parent / "results" / "brats2023_ssa"


# Text prompts to evaluate for SSA brain tumors
SSA_TEXT_PROMPTS = [
    "Brain Tumor",
    "Glioma",
    "Enhancing Tumor",
...
def evaluate_text_prompts(
    ...
    Args:
        ...
        samples: List of BraTSSSASample objects
...
def generate_report(results: Dict, output_dir: Path):
    ...
    csv_path = output_dir / f"{model_name}_ssa_text_prompt_summary.csv"
    ...
    labels_csv_path = output_dir / f"{model_name}_ssa_text_prompt_by_label.csv"
    df_labels.to_csv(labels_csv_path, index=False)
    print(f"Saved by-label results to: {labels_csv_path}")

    # === Detailed JSON ===
    json_path = output_dir / f"{model_name}_ssa_detailed.json"
    with open(json_path, 'w') as f:
        json.dump(results, f, indent=2, default=str)
    print(f"Saved detailed results to: {json_path}")

    # === Markdown Report ===
    report_path = output_dir / f"{model_name}_ssa_report.md"
    with open(report_path, 'w') as f:
        f.write(f"# {model_name.upper()} Evaluation on BraTS2023_SSA (Sub-Saharan Africa)\n\n")
        f.write(f"**Generated**: {results['timestamp']}\n\n")
        f.write(f"**Dataset**: BraTS2023_SSA (Sub-Saharan Africa Brain Tumors)\n\n")
        ...
        # Label explanations
        f.write("\n## Label Definitions\n\n")
        f.write("- **NC**: Non-Enhancing Tumor Core (~91.7% of cases)\n")
        f.write("- **ED**: Edema/Oedema (100% of cases)\n")
        f.write("- **ET**: Enhancing Tumor (100% of cases)\n")

        f.write("\n## Notes\n\n")
        f.write("- Evaluated on middle slices of sampled cases\n")
        f.write("- Text prompts evaluated zero-shot (no prompt tuning)\n")
        f.write("- BraTS2023_SSA: Sub-Saharan Africa gliomas\n")
        f.write("- Lower-quality MRI technology compared to standard BraTS\n")
...
def create_comparison_report(sam3_json: Path, medsam3_json: Path, output_dir: Path):
    ...
    for contrast in CONTRASTS:
        ...
        for prompt in SSA_TEXT_PROMPTS:
    ...
    with open(report_path, 'w') as f:
        f.write("# BraTS2023_SSA Text Prompt-Only Evaluation Report\n\n")
        ...
        f.write("- Text prompts evaluated zero-shot (no prompt tuning)\n")
        f.write("- BraTS2023_SSA: Sub-Saharan Africa gliomas\n")
        f.write("- Lower-quality MRI technology\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="SAM3/MedSAM3 Evaluation on BraTS2023_SSA Dataset (Text Prompts Only)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Maximum number of cases to sample (default: 15, ~20%%)"
    )
    parser.add_argument(
    ...
    if args.create_comparison:
        sam3_json = OUTPUT_DIR / "sam3_ssa_detailed.json"
        medsam3_json = OUTPUT_DIR / "medsam3_ssa_detailed.json"
    ...
    print("=" * 60)
    print("BraTS2023_SSA Evaluation (Sub-Saharan Africa Brain Tumors)")
