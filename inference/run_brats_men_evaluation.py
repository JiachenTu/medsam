#!/usr/bin/env python3
"""
SAM3 and MedSAM3 Evaluation on BraTS2023_MEN (Meningioma) Dataset.
...
Usage:
    ...
    # SAM3 baseline
    python run_brats_men_evaluation.py --model sam3 --max-samples 20

    # MedSAM3 fine-tuned
    python run_brats_men_evaluation.py --model medsam3 --max-samples 20

    # Quick test (5 samples)
    python run_brats_men_evaluation.py --model sam3 --max-samples 5 --contrasts t1c
"""
...
from brats_men_loader import (
    load_brats_men_sample, get_case_ids, load_brats_men_all_labels,
...
# Output directory
OUTPUT_DIR = Path(__file__).parent / "results" / "brats2023_men"


# Text prompts to evaluate for meningioma
MEN_TEXT_PROMPTS = [
    "Brain Tumor",
    "Meningioma",
    "Enhancing Tumor",
...
# Per-label specific prompts (same as MET since labels are identical)
LABEL_TEXT_PROMPTS = {
    "NETC": [
...
def evaluate_text_prompts(
    ...
    Args:
        ...
        samples: List of BraTSMENSample objects
...
def generate_report(results: Dict, output_dir: Path):
    ...
    csv_path = output_dir / f"{model_name}_men_text_prompt_summary.csv"
    ...
    labels_csv_path = output_dir / f"{model_name}_men_text_prompt_by_label.csv"
    df_labels.to_csv(labels_csv_path, index=False)
    print(f"Saved by-label results to: {labels_csv_path}")

    # === Detailed JSON ===
    json_path = output_dir / f"{model_name}_men_detailed.json"
    with open(json_path, 'w') as f:
        json.dump(results, f, indent=2, default=str)
    print(f"Saved detailed results to: {json_path}")

    # === Markdown Report ===
    report_path = output_dir / f"{model_name}_men_report.md"
    with open(report_path, 'w') as f:
        f.write(f"# {model_name.upper()} Evaluation on BraTS2023_MEN (Meningioma)\n\n")
        f.write(f"**Generated**: {results['timestamp']}\n\n")
        f.write(f"**Dataset**: BraTS2023_MEN (Meningioma Brain Tumors)\n\n")
        ...
        # Label explanations
        f.write("\n## Label Definitions\n\n")
        f.write("- **NETC**: Non-Enhancing Tumor Core (~33.9% of cases)\n")
        f.write("- **SNFH**: Surrounding Non-Enhancing FLAIR Hyperintense (~53.6% of cases)\n")
        f.write("- **ET**: Enhancing Tumor (~99.9% of cases)\n")

        f.write("\n## Notes\n\n")
        f.write("- Evaluated on middle slices of sampled cases\n")
        f.write("- Text prompts evaluated zero-shot (no prompt tuning)\n")
        f.write("- BraTS2023_MEN: Meningioma brain tumors\n")
        f.write("- Large dataset (1,139 cases)\n")
...
def create_comparison_report(sam3_json: Path, medsam3_json: Path, output_dir: Path):
    ...
    for contrast in CONTRASTS:
        ...
        for prompt in MEN_TEXT_PROMPTS:
    ...
    with open(report_path, 'w') as f:
        f.write("# BraTS2023_MEN Text Prompt-Only Evaluation Report\n\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="SAM3/MedSAM3 Evaluation on BraTS2023_MEN Dataset (Text Prompts Only)"
    )
    parser.add_argument(
    ...
    if args.create_comparison:
        sam3_json = OUTPUT_DIR / "sam3_men_detailed.json"
        medsam3_json = OUTPUT_DIR / "medsam3_men_detailed.json"
    ...
    print("=" * 60)
    print("BraTS2023_MEN Evaluation (Meningioma Brain Tumors)")
