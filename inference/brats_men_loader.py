"""
BraTS2023_MEN dataset loader for meningioma MRI segmentation.
Handles NIfTI 3D volumes and extracts 2D slices for SAM3/MedSAM3 evaluation.

Labels (same as BraTS2023_MET):
- 0: Background
- 1: NETC (Non-Enhancing Tumor Core) - ~33.9% of cases
- 2: SNFH (Surrounding Non-Enhancing FLAIR Hyperintense) - ~53.6% of cases
- 3: ET (Enhancing Tumor) - ~99.9% of cases
"""
...
BRATS_MEN_ROOT = Path("/shared/BIOE486/SP25/dataset/BrainMVP/BrainMVP-16k/BraTS2023_MEN")

# MRI contrasts available
CONTRASTS = ["t1n", "t1c", "t2w", "t2f"]

# Tumor labels in segmentation mask (same as MET)
TUMOR_LABELS = {
    0: "Background",
...
# Text prompts for meningioma segmentation
BRATS_PROMPTS = {
...
# Standard text prompts to evaluate (includes Meningioma-specific prompt)
...
@dataclass
class BraTSMENSample:
    """A single BraTS-MEN dataset sample (2D slice)."""
    image: np.ndarray          # RGB image (H, W, 3)
    gt_mask: np.ndarray        # Binary mask (H, W)
    case_id: str               # e.g., "BraTS-MEN-00004-000"
...
def get_case_ids(
    ...
    Returns:
        List of case IDs
    """
    all_cases = sorted([d.name for d in BRATS_MEN_ROOT.iterdir() if d.is_dir()])
...
def load_brats_men_sample(
    case_id: str,
    contrast: str = "t1c",
    tumor_label: Optional[str] = None,
    text_prompt: str = "Brain Tumor",
    slice_idx: Optional[int] = None
) -> Optional[BraTSMENSample]:
    """
    Load a single sample from a BraTS-MEN case.
    ...
    Returns:
        BraTSMENSample or None if loading fails
    """
    case_dir = BRATS_MEN_ROOT / case_id
...
def get_cases_with_all_labels(max_samples: int = 10) -> List[str]:
    """
    Find BraTS-MEN cases where NETC, SNFH, and ET are all present in the middle slice.
    ...
    for case_id in all_cases:
        ...
        seg_path = BRATS_MEN_ROOT / case_id / f"{case_id}-seg.nii.gz"
        if not seg_path.exists():
            continue
...
def load_brats_men_dataset(
    ...
    seed: int = 42
) -> Iterator[BraTSMENSample]:
    """
    Generator that yields BraTS-MEN samples for middle slices.
...
def load_brats_men_by_contrast(
    contrast: str,
    max_samples: Optional[int] = None,
    sample_ratio: float = 0.2,
    seed: int = 42
) -> Iterator[BraTSMENSample]:
    """Load BraTS-MEN samples for a single contrast."""
    return load_brats_men_dataset(
        contrasts=[contrast],
...
def load_brats_men_by_label(
    tumor_label: str,
    max_samples: Optional[int] = None,
    sample_ratio: float = 0.2,
    seed: int = 42
) -> Iterator[BraTSMENSample]:
    """Load BraTS-MEN samples for a specific tumor label."""
...
def get_dataset_info() -> dict:
    """Get information about the BraTS-MEN dataset."""
    all_cases = [d.name for d in BRATS_MEN_ROOT.iterdir() if d.is_dir()]
    ...
    return {
        ...
        "root_path": str(BRATS_MEN_ROOT),
    }


if __name__ == "__main__":
    # Test loading
    print("Testing BraTS2023_MEN dataset loader...")
    print("=" * 60)
    ...
    for case_id in case_ids:
        print(f"\n{case_id}:")
        for contrast in CONTRASTS:
            sample = load_brats_men_sample(case_id, contrast)
            if sample:
