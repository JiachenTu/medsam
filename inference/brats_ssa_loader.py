"""
BraTS2023_SSA dataset loader for Sub-Saharan Africa brain tumor MRI segmentation.
Handles NIfTI 3D volumes and extracts 2D slices for SAM3/MedSAM3 evaluation.

BraTS2023_SSA Labels:
- NC (label 1): Non-Enhancing Tumor Core - ~91.7% of cases
- ED (label 2): Edema (Oedema) - 100% of cases
- ET (label 3): Enhancing Tumor - 100% of cases

Key characteristics:
- Sub-Saharan African patient cohort
- Lower-quality MRI technology compared to standard BraTS
- Glioma segmentation (similar to PED, different from MET)
"""
...
BRATS_SSA_ROOT = Path("/shared/BIOE486/SP25/dataset/BrainMVP/BrainMVP-16k/BraTS2023_SSA")
...
TUMOR_LABELS = {
    0: "Background",
    1: "NC",    # Non-Enhancing Core
    2: "ED",    # Edema (Oedema)
    3: "ET",    # Enhancing Tumor
}

# Text prompts for SSA brain tumor segmentation
BRATS_SSA_PROMPTS = {
    "combined": "Brain Tumor",
    "NC": "Non-Enhancing Tumor Core",
...
@dataclass
class BraTSSSASample:
    """A single BraTS-SSA dataset sample (2D slice)."""
    image: np.ndarray          # RGB image (H, W, 3)
    gt_mask: np.ndarray        # Binary mask (H, W)
    case_id: str               # e.g., "BraTS-SSA-00002-000"
    contrast: str              # e.g., "t1c"
    slice_idx: int             # Slice index in volume
    text_prompt: str           # Text prompt for segmentation
    tumor_label: Optional[str] # Label name: "combined", "NC", "ED", "ET"
...
def get_tumor_slices_for_label(
    ...
    Args:
        ...
        label_id: Specific label to look for (1=NC, 2=ED, 3=ET)
...
def get_case_ids(
    ...
    Returns:
        List of case IDs
    """
    all_cases = sorted([d.name for d in BRATS_SSA_ROOT.iterdir() if d.is_dir()])
...
def create_tumor_mask(
    ...
    Args:
        ...
        tumor_label: "NC", "ED", "ET", or None for combined
...
def load_brats_ssa_sample(
    case_id: str,
    contrast: str = "t1c",
    tumor_label: Optional[str] = None,
    text_prompt: str = "Brain Tumor",
    slice_idx: Optional[int] = None
) -> Optional[BraTSSSASample]:
    """
    Load a single sample from a BraTS-SSA case.
    ...
    Returns:
        BraTSSSASample or None if loading fails
    """
    case_dir = BRATS_SSA_ROOT / case_id
...
def load_brats_ssa_all_labels(
    ...
    Returns:
        dict with keys:
            ...
            'nc_mask': Binary mask for NC (label 1)
    ...
    nc_mask = (seg_slice == 1).astype(np.uint8)
...
def get_cases_with_all_labels(max_samples: int = 10) -> List[str]:
    """
    Find BraTS-SSA cases where NC, ED, and ET are all present in the middle slice.
    ...
    for case_id in all_cases:
        ...
        seg_path = BRATS_SSA_ROOT / case_id / f"{case_id}-seg.nii.gz"
        if not seg_path.exists():
            continue
        ...
        has_nc = (mid_slice == 1).sum() > 0
        has_ed = (mid_slice == 2).sum() > 0
        has_et = (mid_slice == 3).sum() > 0

        if has_nc and has_ed and has_et:
            valid_cases.append(case_id)
            print(f"  Found: {case_id} (NC={int((mid_slice==1).sum())}, "
                  f"ED={int((mid_slice==2).sum())}, ET={int((mid_slice==3).sum())})")
...
def load_brats_ssa_dataset(
    ...
    seed: int = 42
) -> Iterator[BraTSSSASample]:
    """
    Generator that yields BraTS-SSA samples for middle slices.
...
def load_brats_ssa_by_contrast(
    contrast: str,
    max_samples: Optional[int] = None,
    sample_ratio: float = 0.2,
    seed: int = 42
) -> Iterator[BraTSSSASample]:
    """Load BraTS-SSA samples for a single contrast."""
    return load_brats_ssa_dataset(
        contrasts=[contrast],
...
def load_brats_ssa_by_label(
    tumor_label: str,
    max_samples: Optional[int] = None,
    sample_ratio: float = 0.2,
    seed: int = 42
) -> Iterator[BraTSSSASample]:
    """Load BraTS-SSA samples for a specific tumor label."""
    prompt = BRATS_SSA_PROMPTS.get(tumor_label, "Brain Tumor")
    return load_brats_ssa_dataset(
        max_samples=max_samples,
        sample_ratio=sample_ratio,
...
def get_dataset_info() -> dict:
    """Get information about the BraTS-SSA dataset."""
    all_cases = [d.name for d in BRATS_SSA_ROOT.iterdir() if d.is_dir()]
    ...
    return {
        ...
        "root_path": str(BRATS_SSA_ROOT),
    }


if __name__ == "__main__":
    # Test loading
    print("Testing BraTS2023_SSA dataset loader...")
    print("=" * 60)
    ...
    for case_id in case_ids:
        print(f"\n{case_id}:")
        for contrast in CONTRASTS:
            sample = load_brats_ssa_sample(case_id, contrast)
            if sample:
    ...
    for label in ["combined", "NC", "ED", "ET"]:
        samples = list(load_brats_ssa_by_label(label, max_samples=2))
        n_with_tumor = sum(1 for s in samples if s.gt_mask.sum() > 0)
        print(f"  {label}: {len(samples)} samples, {n_with_tumor} with tumor")

    # Find cases with all labels
    print("\n" + "-" * 60)
    print("Finding cases with all 3 labels in middle slice...")
    all_label_cases = get_cases_with_all_labels(max_samples=5)

    print("\n" + "=" * 60)
    print("BraTS-SSA loader test complete!")
